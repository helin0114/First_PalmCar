        ...
        mAdapter.setOnLoadMoreListener(new OnLoadMoreListener() {
            ...
            public void onLoadMore(boolean isReload) {
                ...
                if (!CommUtil.checkIsNull(mType)) {
                    if (mLat > 0 && mLon > 0) {
                        mPresenter.loadTasks(mType, mLat, mLon, false, mPageCount, PAGE_SIZE, gpsStatus);
                    } else {
        ...
        if (mLat > 0 && mLon > 0) {
            if (!CommUtil.checkIsNull(mType))
                mPresenter.loadTasks(mType, mLat, mLon,
                        true, mPageCount, PAGE_SIZE, gpsStatus);
        } else {
...
//                    mActivity.mSwipeBackHelper.forward(TaskDetailActivity.class);
        ...
        if (layoutId == R.layout.task_search_popup) {
            ...
            final RadioButton rbLose = (RadioButton) popupView.findViewById(R.id.rb_lose);
...
//                    时间标识（0：当月；1：半年；2：一年）
                    ...
                    if (CommUtil.checkIsNull(id) && CommUtil.checkIsNull(name) && dateFlag > 0) {
                        ...
                        mAdapter.notifyDataSetChanged();
                        mAdapter.setLoadingView(R.layout.load_loading_layout);
                        mPresenter.taskQuery(true, mType, id, name, dateFlag, mPageCount, PAGE_SIZE, gpsStatus);
                        popupWindow.dismiss();
                    }
                }
            });
